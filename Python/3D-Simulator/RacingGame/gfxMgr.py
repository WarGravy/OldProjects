...
class GfxMgr:
    ...
    def setPlayerCamera(self):
        #Detach from Overhead Node
        node = self.sceneManager.getSceneNode('PitchNode1')
        node.detachObject(self.camera)
        #Attach to Player Node
        node = self.sceneManager.getSceneNode('player1node').createChildSceneNode('PlayerCamNode1',
                                                               (-200, 20, 0))
        node.yaw(ogre.Degree(-45))
        node = node.createChildSceneNode('PlayerPitchNode')
    ...
    def setupScene(self):
        ...
        self.camera.nearClipDistance = 50
        ...
        self.sceneManager.setSkyDome (True, "Examples/MountainSkyBox", 5, 8)
