...
ItemType iTreeType::GetItem(ItemType item, bool& found, int& swaps, int& comparions)
// Calls recursive function iRetrieve to search the tree for item.
{
  iRetrieve(root, item, found, swaps, comparions);
  return item;
}
...
//       otherwise found is false and item is unchanged.
{
  if (tree == NULL)
    ...
    comparisons++;      
    iRetrieve(tree->left, item, found, swaps, comparisons); // Search left subtree.
     }
  else if (item > tree->info){
    comparisons++; 
    iRetrieve(tree->right, item, found, swaps, comparisons);// Search right subtree.
     }
  else
  {
...
void FindNode(TreeNode* tree, ItemType item,
     TreeNode*& nodePtr, TreeNode*& parentPtr, int& comparisons)
...
void iDelete(TreeNode*& tree, ItemType item, int&, int&);

void iDeleteNode(TreeNode*& tree, int& swaps, int& comparisons)
...
//       logical predecessor and the predecessor's node is iDeleted.
{
  ...
  else
  {
    ...
    tree->info = data;
    iDelete(tree->left, data, swaps, comparisons);
  }
}

void iDelete(TreeNode*& tree, ItemType item, int& swaps, int& comparisons)
// iDeletes item from tree.
// Post:  item is not in tree.
{
  comparisons++;
  if (item < tree->info)
    iDelete(tree->left, item, swaps, comparisons);   // Look in left subtree.
  else if (item > tree->info, swaps, comparisons)
    iDelete(tree->right, item, swaps, comparisons);  // Look in right subtree.
  else
    iDeleteNode(tree, swaps, comparisons);           // Node found; call iDeleteNode.
}   
...
void iTreeType::PutItem(ItemType item, int& swaps, int& comparions)
// Post: item is in tree.
{
  ...
  FindNode(root, item, nodePtr, parentPtr, comparions);
...
void iTreeType::iDeleteItem(ItemType item, int& swaps, int& comparions)
...
//       matches item. 
{
  ...
  if (nodePtr == root)
    iDeleteNode(root, swaps, comparions);
  else
    if (parentPtr->left == nodePtr)
      iDeleteNode(parentPtr->left, swaps, comparions);
    else iDeleteNode(parentPtr->right, swaps, comparions);
}
