...
int main()
   {
    ...
    //case2 variables
    char tempname[10][20];
    int index = 0, ctr=0, holidayIndex = 0;
            ...
            case 2: // schedule holiday, customer
		     ...
		     //holiday isn't full
		  	if(!full(holidays, holidayname, magicianSize, holidaySize)){
                    ...
                    holidayIndex = searchId(holidayname, holidays, holidaySize);
                    index = 0;
                    while(index<magicianSize){                                 
                        stringcpy(tempname[index], magicians[index].id);
                        index++;
                    }
                    for(index = 0; index < magicianSize; index++){
                            //if there is no match end loop with magician name
                              if(!holidays[holidayIndex].searchSpecial(tempname[index])){
                              ctr = index;
                              index = magicianSize;
                              }
                    }
                    stringcpy(tempItem.specialName, holidayname);
                    stringcpy(tempItem.customerName, custname);
                    index = searchId(tempname[ctr],magicians, magicianSize);
					holidaySize++;
                    magicians[index].insert(tempItem);
                    stringcpy(tempItem.specialName, tempname[ctr]);
                    holidays[holidayIndex].insert(tempItem);
               
                          //update transactions.txt
                    		tout << custname << " was scheduled for " << holidayname << " with " << tempname[ctr] << '.' <<endl;
                          }
                        //holiday is full
                          else{
                    list1.addRear( custname, holidayname );
                             // update transactions.txt
			...
			if( check!= -1 )
               {
				...
				stringcpy(magname,magicians[check].id);
			  	magicians[check].deleteItem();
			  	magicianSize--;

                // check wait list for first customer on that holidayname waiting list
                list1.getFront( custname, holidayname );

                if( !compareString(custname,empty) )
                   {
		  	        ...
		  	        if( !full( holidays, holidayname, magicianSize, holidaySize ) )
                       {
                        ...
                        stringcpy(tempItem.specialName, holidayname);
                        stringcpy(tempItem.customerName, custname);
                        index = searchId(magname,magicians, magicianSize);
					    holidaySize++;
                        magicians[index].insert(tempItem);
                        stringcpy(tempItem.specialName, magname);
                        holidays[holidayIndex].insert(tempItem);
               
                        //update transactions.txt
                        tout << custname << " was scheduled for " << holidayname << " with " << magname << '.' <<endl;
                       }
