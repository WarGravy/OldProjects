	  ...
	  for( row = 0; row < 7; row++)
	   {
		...
		{
		//project2: change arrays to the appropriate structure arrays
		//add card
...
void shuffleDeck(char cards[2][MAX_COL])
 ...
 while(count < 160)
	...
	if(randomNum < 52 && randomNum2 <52 && randomNum != randomNum2)
	  {
	//project2: storing will need to be changed also use string copy for color and suit names
	//store in temp for trade
	temp1 = cards[0][randomNum];
	temp2 = cards[1][randomNum];
	//project2: trading will need to be changed(note also needs to include the change in the color of the card
	//trade
  ...
  fout.open(fileName);
	...
	while(index < 24)
	  ...
	  dummy2 = waste[1][index];
		//project2: change from A,1,2...8,9,J,Q,K to 1-13
		...
		//project2: get rid of cases and output the suit name
		//switch statements for names
		switch(dummy2)
		  {
		  case 'S':
	...
	for(rowIndex = 0; rowIndex < 7; rowIndex++)
	  {
		...
		//project2: this is all going to have to be redone
		//switch statements for names
		switch(dummy1)
		{
		case '0':
...
void clearTable( char array[7][7][MAX_NAME])
  ...
  for(xIndex = 0; xIndex < 7; xIndex++)
	...
	//project2: fix so it contains the word Empty, not E
...
void outputScreen(char waste[2][24], char tab[7][7][MAX_NAME])
  ...
  //output to console the first card in waste
	/*project2: Change all the below so that it reads the cards name, and value from 1-13 from the struct and outputs it's name*/
...
void readFile( char wastePile[2][24], char fileName[MAX_NAME], char tab[7][7][MAX_NAME])
  ...
  fin.clear();
  fin.open(fileName);
	//project 2: loading will be different since there will be structures
    if (fin.good())
	{
...
void iniDeck(char deckOfCards[2][MAX_COL])
  {
	...
	int diamonds = 26;
	int clubs = 39;
	//project2: the structure array will be initialized instead of a 2d array
