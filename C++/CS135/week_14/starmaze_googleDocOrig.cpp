...
//initialize function/variables
    //variables
    int levelX = SCRN_MAX_X - 20;
    int levelY = SCRN_MAX_Y;
    double cycle;
    int newY = 0;
    int input;
    int cycleTime = 1;
...
//MASTER LOOP----------------------------------------------------------------
    ...
    for(cycle = 0.0; continueGame; cycle++)
     //MASTER START{
     {
     ...
     switch(difficultySet)
        ...
        if (xyLoc[0][XY_LOC_ARR_MAX] == 1)
          //{
          {
            //added in step 6* make sure the game hasn't already ended
            if( continueGame )
            {
    ...
    //if true
      //{
      {
        //prompt for username
          ...
          setColor(COLOR_WHITE, COLOR_BLUE, true);
          clearScreen( MENU_MIN_X, MENU_MIN_Y, MENU_MAX_X, MENU_MAX_Y);
          promptForStringAt(centerX, centerY, "Please Enter your name:", username);         
        //set new score
         //function: scoring
...
//output blank scores and names
    ...
    fout << blankName << COM << blankScore << COM;
    //}
    }
//close file
fout.close();
...
void initializeArr ( char gameArr[MAX_HEIGHT][MAX_WIDTH], int difficulty, int xyLocArr[MAX_HEIGHT][MAX_WIDTH] )
{
   ...
   int count = 0, hard = 1, medium = 2;

   // initialize rand function
   srand ( time(NULL) );

   // initialize gameArr to all 'space'
   for( rowIndex = 0; rowIndex < MAX_HEIGHT; rowIndex++ )
      {
       for( colIndex = 0; colIndex < MAX_WIDTH; colIndex++ )
          {
