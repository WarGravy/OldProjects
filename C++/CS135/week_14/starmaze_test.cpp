...
*/
bool checkForScore( char gameArr[MAX_HEIGHT][MAX_WIDTH], int xyArrLoc[MAX_HEIGHT][MAX_WIDTH],int yCursorLoc);
...
//MASTER LOOP----------------------------------------------------------------
    ...
    for(cycle = 0; continueGame; cycle++)
     ...
     //check if cursor is at the same location as an asteroid calculate score
       ...
       if(checkForScore(level, xyLoc,cursorY))
        //{
        {
     ...
     //{
       {
        ...
        if(xyArrLoc[1][col] == yCursorLoc)
          //{   
            {
            setColor(COLOR_RED, COLOR_BLACK, true);
             gameArr[yCursorLoc][col + 1] = '&';
             //return true
             return true;
          //}
          }
