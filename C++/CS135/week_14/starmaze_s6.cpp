...
const char SPACE = ' ';
const int CURSOR_X_LOC = 24;
const char COM = ',';
const char FILENAME[MAX_WIDTH] = "topscores.txt";
const int ARR_TERMINATOR = 999;
...
*/
void topScoreMenu( const char fileName[] );
...
*/
void scoring(const char fileName[] , int highscore, char username[]);
...
Dependencies:none
*/
void writeHighScore(const char fileName[] , int highscore, char username[], int scores[]);
...
*/
bool checkIfHighScore(const char fileName[], int score);
...
//initialize function/variables
    //variables
    int levelX = SCRN_MIN_X + 30;
    int levelY = SCRN_MIN_Y;
    int cycle;
    int newY = 0;
    int input;
    int cycleTime = 1;
    ...
    int cursorY = centerY;
    char endgame[] = "End Game - Press Any Key";
     ...
     switch(difficultySet)
         //START { for difficulty switch
         {
           ...
           case 1:
             //move level
               //function: move array
               moveArr( level, xyLoc, levelX, levelY );
           //break;
           break;
...
if( input == KB_ESCAPE)
    {
    setColor(COLOR_BLUE, COLOR_GREEN, true);
    clearScreen(SCRN_MIN_X, SCRN_MIN_Y, SCRN_MAX_X, scoreBoardY);
    setColor(COLOR_CYAN, COLOR_BLUE, true);
    clearScreen(MENU_MIN_X, SCRN_MIN_Y, MENU_MAX_X, scoreBoardY);
    printStringAt(centerX , centerY, endgame, "CENTER");
    waitForInput(FIXED_WAIT);
    }
 // check if score is a high score
    //function: checkIfHighScore
    if(checkIfHighScore(FILENAME, total) )
    //if true
      //{
      {
        //prompt for username
          ...
          setColor(COLOR_WHITE, COLOR_BLUE, true);
          clearScreen( MENU_MIN_X, MENU_MIN_Y, MENU_MAX_X, MENU_MAX_Y);
          promptForStringAt(centerX / 2, centerY, "Please Enter your name:", username);         
        //set new score
         //function: scoring
         scoring(FILENAME, total, username);
      //}
      }
...
//sort high scores
    ...
    //stream ofstream with numbers in correct order including endlines
      fout << username << COM << endl;
      fout << highscore << COM << endl;
       // write first place to second
          fout << place1 <<COM << endl;
          fout << scores[0]<<COM<< endl;
       //write second place to third
          fout << place2 << COM<< endl;
          fout << scores[1] << COM<< endl;
       //write third place to forth
          fout << place3 << COM<< endl;
          fout << scores[2] << COM<< endl;
       //write forth place to fifth
          fout << place4 << COM<< endl;
          fout << scores[3]<< endl;
    //break
    break;
    //case 1
...
//output blank scores and names
    ...
    fout << blankName << COM << blankScore << COM << endl;
    fout << blankName << COM << blankScore << COM << endl;
    fout << blankName << COM << blankScore << COM << endl;
    fout << blankName << COM << blankScore << COM << endl;
    fout << blankName << COM << blankScore;
//close file
fout.close();
//return true
return true;
...
void initializeArr ( char gameArr[MAX_HEIGHT][MAX_WIDTH], int difficulty, int xyLocArr[MAX_HEIGHT][MAX_WIDTH] )
{
   ...
   int count = 0, hard = 3, medium = 2;

   // initialize rand function
   srand ( time(NULL) );

   // initialize gameArr to all 'space'
   for( rowIndex = 0; rowIndex < 21; rowIndex++ )
      {
       for( colIndex = 0; colIndex < 60; colIndex++ )
          {
           gameArr[rowIndex][colIndex] = SPACE;
          }
      }

   // insert '#' characters into random locations gameArr
   for( index = 0; index < MAX_ASTEROIDS; index++ )
      {
       ranX = rand() % 60;
       ranY = rand() % 21;
       //original: gameArr[ranY][ranX] = '#';
       //added by zack:
//-----------------------------------------------
       if(difficulty == hard)
         {
         gameArr[ranY][ranX] = '#';
         //set gameArr x and y locations
         xyLocArr[0][index] = ranX + 30;
         xyLocArr[1][index] = ranY;
         }
         ...
         gameArr[ranY][ranX + 1] = '#';
         //set gameArr x and y locations
         xyLocArr[0][count] = ranX + 30;
         xyLocArr[1][count] = ranY;
         xyLocArr[0][count+1] = ranX + 31;
         xyLocArr[1][count+1] = ranY;
         count += 2;
         }
        else
        {
         ...
         xyLocArr[0][count+2] = ranX + 31;
         xyLocArr[1][count+2] = ranY + 1;
         
         xyLocArr[0][count+3] = ranX + 30;
         xyLocArr[1][count+3] = ranY + 1;
         count += 4;
        }
     }
        //add ARR_TERMINATOR
        xyLocArr[0][count] = ARR_TERMINATOR;
        xyLocArr[1][count] = ARR_TERMINATOR;
}
bool checkForScore( int xyArrLoc[MAX_HEIGHT][MAX_WIDTH],int yCursorLoc)
{
//initialize variables
...
for(col = 0; xyArrLoc[0][col] != ARR_TERMINATOR; col++)
    //{
    {  
